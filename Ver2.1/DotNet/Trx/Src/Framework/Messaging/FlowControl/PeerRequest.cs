...
namespace Trx.Messaging.FlowControl {
	...
	public class PeerRequest {
		...
		private static VolatileSequencer _sequencer = new VolatileSequencer();
		...
		private int _id = PeerRequest._sequencer.Increment();
		...
		public void WaitResponse( int timeout) {
			...
			if ( timeout < 1) {
				throw new ArgumentOutOfRangeException("timeout", timeout, "Must be greater tha zero.");
			}
		...
		public override string ToString() {
			...
			if ( _responseMessage == null) {
				...
				TimeSpan t = _responseDateTime -  _requestDateTime;

				return string.Format( "{0} request for peer {1}, response received in {2}ms",
					( _expired ? "Expired" : "Successful" ), _id,
					( ( TimeSpan)(_responseDateTime - _requestDateTime)).TotalMilliseconds.ToString());
			}
		}
		#endregion
	}
