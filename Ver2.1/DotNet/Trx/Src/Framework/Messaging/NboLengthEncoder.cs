...
namespace Trx.Messaging {
	...
	/// <summary>
	/// This class implements a length encoder in NBO (Network Byte Order).
	/// </summary>
	/// <remarks>
	...
	/// </remarks>
	public class NboLengthEncoder : ILengthEncoder {

		// One for each supported size, if more are required only
		// enlarge the instances array and set new lengths in _lengths.
		private static volatile NboLengthEncoder[] _instances = {
			null, null, null
		};
		private static int[] _lengths = { 255, 65535, 16777215};
		...
		/// </param>
		private NboLengthEncoder( int lengthsIndex) {
		...
		#region Methods
		/// <summary>
		/// It returns an instance of <see cref="NboLengthEncoder"/>
		/// class.
		/// </summary>
		...
		/// </param>
		/// <returns>
		/// An instance of <see cref="NboLengthEncoder"/>.
		/// </returns>
		/// <exception cref="ArgumentOutOfRangeException">
		/// It's thrown when <paramref name="maximumLength"/> holds an invalid value.
		/// </exception>
		public static NboLengthEncoder GetInstance( int maximumLength) {
			...
			if ( _instances[index] == null) {
				lock ( typeof( NboLengthEncoder)) {
					if ( _instances[index] == null) {
						_instances[index] = new NboLengthEncoder( index);
					}
				}
			}
